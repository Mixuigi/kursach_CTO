...
def admin_approve_request_view(request):
    enquiry = models.Request.objects.all().filter(status='В ожидании')
...
def admin_report_view(request):
    reports = models.Request.objects.all().filter(Q(status="Отремонтировано") | Q(status="Можно забирать"))
    dict = {
        'reports': reports,
    }
...
def customer_dashboard_view(request):
    ...
    work_in_progress = models.Request.objects.all().filter(customer_id=customer.id, status='Ремонт').count()
    work_completed = models.Request.objects.all().filter(customer_id=customer.id).filter(
        Q(status="Отремонтировано") | Q(status="Выпущено")).count()
    new_request_made = models.Request.objects.all().filter(customer_id=customer.id).filter(
        Q(status="В ожидании") | Q(status="Одобрено")).count()
    bill = models.Request.objects.all().filter(customer_id=customer.id).filter(
        Q(status="Отремонтировано") | Q(status="Можно забирать")).aggregate(Sum('cost'))
    print(bill)
    dict = {
...
def customer_view_request_view(request):
    customer = models.Customer.objects.get(user_id=request.user.id)
    enquiries = models.Request.objects.all().filter(customer_id=customer.id, status="В ожидании")
...
def customer_view_approved_request_view(request):
    customer = models.Customer.objects.get(user_id=request.user.id)
    enquiries = models.Request.objects.all().filter(customer_id=customer.id).exclude(status='В ожидании')
...
def mechanic_dashboard_view(request):
    ...
    work_in_progress = models.Request.objects.all().filter(mechanic_id=mechanic.id, status='Ремонт').count()
    work_completed = models.Request.objects.all().filter(mechanic_id=mechanic.id, status='Отремонтировано').count()
    new_work_assigned = models.Request.objects.all().filter(mechanic_id=mechanic.id, status='Одобрено').count()
    dict = {
        'work_in_progress': work_in_progress,
        'work_completed': work_completed,
...
def mechanic_salary_view(request):
    ...
    workdone = models.Request.objects.all().filter(mechanic_id=mechanic.id).filter(
        Q(status="Отремонтировано") | Q(status="Можно забирать"))
