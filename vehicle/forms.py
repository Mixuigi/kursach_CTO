...
class AdminRequestForm(forms.Form):
    ...
    customer = forms.ModelChoiceField(queryset=models.Customer.objects.all(), empty_label="Клиент",
                                      to_field_name='id')
    mechanic = forms.ModelChoiceField(queryset=models.Mechanic.objects.all(), empty_label="Механик",
                                      to_field_name='id')
...
class AdminApproveRequestForm(forms.Form):
    mechanic = forms.ModelChoiceField(queryset=models.Mechanic.objects.all(), empty_label="Mechanic Name",
                                      to_field_name='id')
    cost = forms.IntegerField()
    stat = (('В ожидании', 'В ожидании'), ('Одобрено', 'Одобрено'), ('Выпуск', 'Выпуск'))
...
class MechanicUpdateStatusForm(forms.Form):
    stat = (('Одобрено', 'Одобрено'), ('Ремонт', 'Ремонт'), ('Отремонтировано', 'Отремонтировано'))
    status = forms.ChoiceField(choices=stat)
...
presence_choices = (('Присутствует', 'Присутствует'), ('Отсутствует', 'Отсутствует'))
